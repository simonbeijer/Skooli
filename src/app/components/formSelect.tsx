    ...
    <div className="space-y-2">
      ...
      <div className="relative" ref={selectRef}>
        <button
          ...
          className={`flex h-10 w-full items-center justify-between rounded-md border bg-white px-3 py-2 text-base ring-offset-white placeholder:text-gray-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm transition-all duration-200 ${
            error 
        ...
        {isOpen && (
          <div 
            className="absolute z-50 mt-1 w-full rounded-md border border-muted bg-white py-1 shadow-xl max-h-60 overflow-auto"
            role="listbox"
          ...
          >
            {options.map((option) => (
              <button
                ...
                className={`w-full px-3 py-2 text-left text-sm text-foreground transition-colors duration-150 ${
                  option.value === value 
                    ? 'bg-primary text-white' 
                    : 'hover:bg-primary/10 focus:bg-primary/10 hover:text-foreground'
                } focus:outline-none`}
                role="option"
